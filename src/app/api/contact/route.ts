...
function buildHtml(payload: {
  ...
  const { name, emailDisplay, emailRaw, phone, subject, message } = payload;
                  ...
                  <tr>
                    <td style="padding:24px 28px;">
                      ...
                      <p style="margin:0;font-size:15px;line-height:1.6;color:rgba(246,243,229,0.82);">
                        Responde directamente a <a href="mailto:${emailRaw}" style="color:#f6f3e5;text-decoration:none;font-weight:600;">${emailDisplay}</a>. Inclui una propuesta personalizada y, si corresponde, coordina una videollamada o visita al atelier Altessa.
                      </p>
                    </td>
                  </tr>
                </table>
...
export async function POST(request: Request) {
  try {
    ...
    const sanitizedSubject = subjectInput
      .replace(/[\r\n]+/g, " ")
      .slice(0, 120);
    ...
    const safeSubject = escapeHtml(
      sanitizedSubject || "Consulta desde el sitio web"
    );
